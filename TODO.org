...
da komponente budu =stable_vector=, da se zove destruktor svake

range based for loop

TEK KADA SVE RADI:
    - window apstraction
    - organizacija, materijali, rename update->onUpdate, startUp->init(), singleton na stvari
    - log na entity
    - Material map da bude safe (?opratori, copy construtor?)
    - move semantics
    - organizacija fajlova:
        - ukloniti types.h
        - namestiti dobar vendor inlcude stuff
